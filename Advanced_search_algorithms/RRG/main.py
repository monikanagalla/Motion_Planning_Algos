...
if __name__ == "__main__":
    ...
    RRG_planner = RRG(map_array, start, goal)

    # Search with PRM
    # s = PRM_planner.sample(n_pts=1000, sampling_method="uniform")
    # PRM_planner.search(start, goal, s)
    # s = PRM_planner.sample(n_pts=1000, sampling_method="random")
    # PRM_planner.search(start, goal, s)
    # s = PRM_planner.sample(n_pts=2000, sampling_method="gaussian")
    # PRM_planner.search(start, goal, s)
    # s = PRM_planner.sample(n_pts=20000, sampling_method="bridge")
    # PRM_planner.search(start, goal, s)

    # # Search with RRT and RRT*
    # RRT_planner.RRT(n_pts=1000)
    # RRT_planner.RRT_star(n_pts=2000)
    RRG_planner.RRG(n_pts=2000)
